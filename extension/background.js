...
async function checkDateOpen(theaterCode, date, movieKeyword) {
  ...
  try {
    ...
    if (sheetOpen) {
      ...
      // 4c: "극장선택" 버튼 클릭 (class="fill-black")
      const s4c = await run(tab.id, () => {
        // 정확한 클래스로 찾기
        const btn = document.querySelector('button.fill-black') ||
                    document.querySelector('.btn-100.fill-black') ||
                    document.querySelector('[class*="fill-black"]');
        if (btn) {
          btn.scrollIntoView({ behavior: 'instant' });
          btn.click();
          return 'clicked: ' + btn.textContent?.trim().substring(0, 10);
        }
        // 텍스트로 찾기
        for (const el of document.querySelectorAll('button')) {
          if (el.textContent?.trim() === '극장선택') {
